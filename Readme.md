...
🎯 **SPF** addresses time-varying stochastic optimization (TV-SO) problems as the stochastic couterpart of conventional time-varying optimization (TV-O) challenges ($\min_{(x_t)_{t \in [0,T]}} f(\mathbf{x}_t, t), ~\forall t \in [0, T]$).
